...
Meteor.startup(() => {
  ...
  if (!Spiderable.bufferSizeInBytes) {
	Spiderable.bufferSizeInBytes = 1000 * 1024 * 1024; //1GB default
  }
  if (!_.isNumber(Spiderable.bufferSizeInBytes)) {
    throw new Meteor.Error('Bad Spiderable.bufferSizeInBytes');
  }

  if (!Spiderable.requestTimeoutInMilliseconds) {
	Spiderable.requestTimeoutInMilliseconds = 30 * 1000; //30 seconds default
  }
  if (!_.isNumber(Spiderable.requestTimeoutInMilliseconds)) {
    throw new Meteor.Error('Bad Spiderable.requestTimeoutInMilliseconds');
  }
  ...
  if ((/\?.*_escaped_fragment_=/.test(req.url) || _.any(Spiderable.userAgentRegExps, (re) => {
    ...
    if (cached) {
      ...
      child.exec(fullCommand, {
        timeout: Spiderable.requestTimeoutInMilliseconds,
        maxBuffer: Spiderable.bufferSizeInBytes
      }, (error, stdout, stderr) => {
        bindEnvironment(() => {
          if (!error) {
            if (stdout.length) {
